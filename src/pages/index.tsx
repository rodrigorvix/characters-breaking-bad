...
export default function Home({characterList, episodiesList}) {
  ...
  return (
    <>
      <Header/>
      <CharacterCardList characterList={characterList} episodiesList={episodiesList}/>
    </>
  )
}
...
export const getStaticProps: GetStaticProps = async () => {
  ...
  const characterList = await responseCharacters.map(({char_id,name,img,portrayed, nickname}) => {
    return {
      char_id,
      name,
      img,
  ...
  const episodiesList = await responseEpisodies.map(({title,season,air_date,characters,episode}) => {
    return {
      title,
      season,
      air_date,
