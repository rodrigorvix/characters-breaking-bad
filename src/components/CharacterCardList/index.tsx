...
export function CharacterCardList (props:CharacterCardListPropsType) {
  ...
  const getDetailsCharacter = (name,nickname:string) => {
    const result = props.episodiesList.filter(episode => {
      return episode.characters.indexOf(name) !== -1 || 
      episode.characters.some(character => character.toUpperCase().includes(nickname.toUpperCase()));
    })  
    return result;
  }

  return (
    <main className={styles.characterCardListContainer}>
     
      <Grid container spacing={2}>
      {  props.characterList.map(({char_id,name,img,portrayed,nickname}) => {
